    ...
    def do_CONNECT(self):
        ...
        try:
            ...
            if soc:
                ...
                self.wfile.write(ensure_binary(self.protocol_version +
                                               " 200 Connection established\r\n"))
                self.wfile.write(ensure_binary(f"Proxy-agent: {self.version_string()}\r\n"))
                self.wfile.write(ensure_binary("\r\n"))
    ...
    def do_GET(self):
        ...
        try:
            if scm == 'http':
                ...
                if soc:
                    self.log_request()
                    soc.send(f"{self.command} {urllib.parse.urlunparse(('', '', path, params, query, ''))} " +
                ...
                try:
                    ...
                    if self.command == "GET":
                        ftp.retrbinary(f"RETR {path}", self.connection.send)
                    ftp.quit()
...
def daemonize_part2(logger):
    class DevNull():
        def __init__(self):
            self.fd = os.open(os.path.devnull, os.O_WRONLY)
...
def daemonize(logger, opts):
    ...
    if os.name == "nt":
        ...
        for path in sys.path:
            if os.path.exists(os.path.join(path, "python.exe")) is True:
        ...
        for opt, value in opts:
            if opt == "-d":
                ...
                cmdline = cmdline + f" {str(opt)} {str(value)}"
...
def main():
    ...
    if args:
        ...
        for name in args:
            ...
            logger.log(logging.INFO, f"Accept: {client} ({name})")
