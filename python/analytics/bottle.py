...
class HTTPResponse(BottleException):
    ...
    def __init__(self, output='', status=200, header=None):
        super(BottleException, self).__init__(f"HTTP Response {status}")
...
class Router:
    ...
    def build(self, _name, *anon, **args):
        ...
        if args: url += ['?', parse.urlencode(args)]
        return ''.join(url)
...
class Bottle:
    ...
    def get_url(self, routename, **kargs):
        ...
        return parse.urljoin(urljoin('/', scriptname), location)
    ...
    def _cast(self, out, request, response, peek=None):
        ...
        return self._cast(HTTPError(500, f'Unsupported response type: {type(first)}'),
                          request, response)
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            if not self.catchall: raise
            err = f"<h1>" \
                  f"Critical error while processing request: {environ.get('PATH_INFO', '/')}" \
                  f"</h1>"
            if DEBUG:
                err += f'<h2>Error:</h2>\n<pre>{repr(e)}</pre>\n'
...
class Request(threading.local, DictMixin):
    ...
    def urlparts(self):
        ...
        path = parse.quote(parse.urljoin(spath, rpath))
        return parse.SplitResult(http, host, path, env.get('QUERY_STRING'), '')
    ...
    def fullpath(self):
        ...
        return parse.unquote(self.urlparts[2])
...
def redirect(url, code=303):
    ...
    location = parse.urljoin(request.url, url)
...
class ServerAdapter:
    ...
    def __init__(self, host='127.0.0.1', port=8082, **config):
        self.options = config
        self.host = host
        self.port = int(port)
    ...
    def __repr__(self):
        args = ', '.join([f'{k}={repr(v)}' for k, v in list(self.options.items())])
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8082,
